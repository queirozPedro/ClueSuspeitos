...
def iniciar_treino():
    ...
    clf = MLPClassifier(solver='lbfgs', alpha=1e-5, hidden_layer_sizes=(5, 8), random_state=1, max_iter=1000000)
    clf1 = MLPClassifier(solver='lbfgs', alpha=1e-5, hidden_layer_sizes=(5, 10), random_state=1, max_iter=1000000)
    clf2 = MLPClassifier(solver='lbfgs', alpha=1e-5, hidden_layer_sizes=(5, 12), random_state=1, max_iter=1000000)
    ...
    string = [
        ...
        f"Resultado\n",
        f"Acertou tudo: {(acertos/jogos_teste)*100:.2f}%\n",
        f"Acertou somente Arma e Lugar: {(acertos_arma_lugar/jogos_teste)*100:.2f}%\n",
        f"Acertou somente Arma e Suspeito: {(acertos_arma_suspeito/jogos_teste)*100:.2f}%\n",
        f"Acertou somente Lugar e Suspeito: {(acertos_lugar_suspeito/jogos_teste)*100:.2f}%\n",
        f"Acertou apenas a Arma: {(acertos_arma/jogos_teste)*100:.2f}%\n",
        f"Acertou apenas o Lugar: {(acertos_lugar/jogos_teste)*100:.2f}%\n",
        f"Acertou apenas o Suspeito: {(acertos_suspeito/jogos_teste)*100:.2f}%\n",
        f"Errou tudo: {(erros_totais/jogos_teste)*100:.2f}%\n"
    ]
...
def iniciar_treino_venn():
    ...
    clf = MLPClassifier(hidden_layer_sizes=(16, 8), activation='relu', solver='adam', alpha=0.0001, max_iter=1000000, random_state=1)
    clf1 = MLPClassifier(hidden_layer_sizes=(20, 10), activation='relu', solver='adam', alpha=0.0001, max_iter=1000000, random_state=1)
    clf2 = MLPClassifier(hidden_layer_sizes=(24, 12), activation='relu', solver='adam', alpha=0.0001, max_iter=1000000, random_state=1)

    # clf = MLPClassifier(solver='lbfgs', alpha=1e-5, hidden_layer_sizes=(8, 4), random_state=1, max_iter=1000000)
    # clf1 = MLPClassifier(solver='lbfgs', alpha=1e-5, hidden_layer_sizes=(10, 5), random_state=1, max_iter=1000000)
    # clf2 = MLPClassifier(solver='lbfgs', alpha=1e-5, hidden_layer_sizes=(12, 6), random_state=1, max_iter=1000000)
...
def iniciar_treino_venn_loop():
    print(f"\n\nCaracteristicas do Treino")
    entradas = int(input("Quantidade de Jogos para Treinamento: "))
    jogos_teste = int(input("Quantidade de Testes: "))
    print(f"Sobre os Treinos")
    inicio, fim = int(input("Porcentagem inicial: ")), int(input("Porcentagem final: "))
    intervalo = int(input("Intervalo entre as porcentagens: "))
    ...
    while inicio <= fim:
        ...
        for n in range(entradas):
            # Vou criar o crime e a tabela
            crime, tabela = criar_cenario(inicio, inicio)
            # Distribuir o crime em seu respectivos y
            arma_y.append(crime[0])
            lugar_y.append(crime[1])
            suspeito_y.append(crime[2])
        ...
        string = [
            f"\n\nCom um total de {entradas} entradas de jogos\n",
            f"Para um total de {jogos_teste} testes automáticos e aleatórios\n",
            f"Com chance de {inicio}% de Corromper o 1\n",
            f"Com chance de {inicio}% de Corromer o -1\n",
            f"Resultado\n",
            f"Acertou tudo: {acertos} ou {(acertos/jogos_teste)*100:.2f}%\n",
            f"Acertou somente Arma e Lugar: {acertos_arma_lugar} ou {(acertos_arma_lugar/jogos_teste)*100:.2f}%\n",
            f"Acertou somente Arma e Suspeito: {acertos_arma_suspeito} ou {(acertos_arma_suspeito/jogos_teste)*100:.2f}%\n",
        ...
        string = [
            f"Total de entradas: {entradas}\n",
            f"Total de testes: {jogos_teste}\n",
            f"Corrupção de 1 e -1: {inicio}%, {inicio}%",
        ]
        plt.text(-1, -0.8, "".join(string), fontsize=10)
        plt.text(0.5, -0.5, f"Erros: {round((erros_totais/jogos_teste)*100, 2)}", fontsize=12)
        plt.savefig(f'{entradas}_entradas_e_{jogos_teste}_testes_{inicio}%_para_1_e_{inicio}%_para_-1')
        plt.clf()
        inicio += intervalo

def main(): 
    iniciar_treino_venn_loop()
